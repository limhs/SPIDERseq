...
def run_multi(thread_function, threads, df, WD, input,sample, target_info, min_base_ratio, other_args=() ):
	Ps=[]
	unit=len(df.index)/threads
	...
	for n in range(1,threads+1):
		...
		procs=Process(target=thread_function, args=(WD, input, sample,n, target_info, df.loc[df.index[stt:end+1]], min_base_ratio)+other_args )
		Ps.append(procs)
		procs.start()
