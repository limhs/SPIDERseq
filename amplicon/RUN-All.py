...
def add_args():
	...
	parser.add_argument("-u", "--min_umi_pair", type=int, default=1, help="at least depth of UID")
	parser.add_argument("-m" ,"--min_base_ratio", type=float ,default=0.7, help="minimum frequency of major base per CID")
	parser.add_argument("-n", "--min_nread", type=int, default=3, help="at least number of reads per cluster")
...
def main():
    ...
    ### make peer-to-peer network
	if type=="c":
		print "-------pair_save / cut UIDs less than {}".format(str(cutoff))
	...
	if indel_mode:
		...
		if type=="c":
			...
			base_count.cluster_based_indel(WD, input, sample, n_core, target_info, min_base_ratio, min_nread, min_nUID, indelvcf, align_index)
		...
		if type=="c":
			...
			base_count.cluster_based_call(WD, input, sample, n_core, target_info, min_base_ratio, min_nread, min_nUID)
