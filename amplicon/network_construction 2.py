...
def network_const(WD, sample, target_info,  n_core, cycle=8):
	...
	for n in range(n_core):
		while True:
			try:
				out_chunk=pd.read_csv("{}/cluster2/{}.{}.cluster.txt".format(WD,  sample,str(n)), sep='\t', index_col="locus_id")
			except IOError:
				time.sleep(30)
				continue
			break
		out_df=out_df.append(out_chunk)
		os.system("rm {}/cluster2/{}.{}.cluster.txt".format(WD, sample,str(n)), str(n)) 


	out_df.to_csv("{}/cluster2/{}.cluster.txt".format(WD, sample), sep='\t', index=False)

	

def thread_function(WD, sample,n,  targets_chunk, cycle, filtered, count_dic):
	for target in targets_chunk:
		out_cluster=open("{}/cluster2/{}.{}.cluster.txt".format(WD, sample,str(n)), "w")
		out_cluster.write("UMI_R1\tR1_size\tUMI_R2\tR2_size\ttarget\tSegment\torigin\tNumber_of_branch\tpair_list\tnPairs\tnReads\n")
		...
		U1_df.loc[:, U1_df.columns.isin(["R1_UMI", "linked_UMI_R2s", "size", "target"])].to_csv("{}/cluster2/{}.{}.u1.tsv".format(WD,target, sample),  sep='\t')
		U2_df.loc[:, U2_df.columns.isin(["R2_UMI", "linked_UMI_R1s", "size", "target"])].to_csv("{}/cluster2/{}.{}.u2.tsv".format(WD,target, sample),  sep='\t')
		...
		for seed in seedlist:
			...
			### save
			outform='\t'.join([";".join(u1_cluster),str(len(u1_cluster)), ";".join(u2_cluster),str(len(u2_cluster)), target, origin_side, origin_umi, str(maxi), ";".join(pair_list), str(len(pair_list)), str(nreads)])
			out_cluster.write(outform+'\n')
